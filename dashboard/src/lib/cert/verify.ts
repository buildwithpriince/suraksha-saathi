/** docs/04 certificate verification (steps 1-9). Mirrors backend app/crypto/certificates.py. */
import {
  parseAttestation,
  parseCertificate,
  parseRevocationList,
...
export async function verifyCertificate(
  token: string,
  { rootPublicKey, now, revocationList = null }: VerifyOptions,
  ...
  try {
    att = await verifyToken(cert.att, "SA1", rootPublicKey, parseAttestation);
  ...
  try {
    await verifySignature(deviceKey, split.signature, split.signingInput);
  } catch {
  ...
  const revocations = revocationList === null ? null : await verifyRevocationList(revocationList, rootPublicKey);
...
export async function verifyRevocationList(
  token: string,
  rootPublicKey: Uint8Array,
): Promise<RevocationListBody | null> {
  try {
    return await verifyToken(token, "SR1", rootPublicKey, parseRevocationList);
  } catch {
    return null;
  }
}
...
/** docs/04 UI mapping. */
export function statusLabel(status: VerifyStatus): { label: string; tone: StatusTone } {
  switch (status) {
    case "VALID":
